...
const ChatMessagesScreen = () => {
    ...
    if (recepientData?.myboarding) {
      ...
      try {
        const response = await axios.put(
          `http://172.20.10.2:8000/api/boardings/${boarding}/addtenant`,
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `http://172.20.10.2:8000/api/user/messages/${userId}/${recepientId}`
      );
  ...
  const fetchRecepientData = async () => {
    try {
      const response = await fetch(
        `http://172.20.10.2:8000/api/user/user/${recepientId}`
      );
  ...
  const handleSend = async (messageType, imageUri) => {
    ...
    try {
      ...
      const response = await fetch("http://172.20.10.2:8000/api/user/messages", {
        method: "POST",
        body: formData,
      });
  ...
  const fetchboardingdata = async () => {
    try {
        const response = await fetch(
            `http://172.20.10.2:8000/api/user/getownboardings/${userId}`
        );
