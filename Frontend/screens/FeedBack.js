...
const FeedBack = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserDetails = async () => {
      try {
        const response = await fetch(`http://192.168.1.6:8000/api/user/user/${userId}`);
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`http://192.168.1.6:8000/api/comments/boardings/${boardingId}/comments`);
      const data = await response.json();
      if (response.ok) {
  ...
  const handleDeleteComment = () => {
    if (selectedCommentId) {
      axios.delete(`http://192.168.1.6:8000/api/comments/comments/${selectedCommentId}`)
      .then(() => {
  ...
  const handleUpdateComment = () => {
    ...
    if (selectedCommentId) {
      axios.put(`http://192.168.1.6:8000/api/comments/comments/${selectedCommentId}`, {
        text: editedText,
