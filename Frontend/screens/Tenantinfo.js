...
const Tenantinfo = ({ route }) => {
  ...
  useEffect(() => {
    const fetchTenantInfo = async () => {
      try {
        const response = await axios.get(`http://192.168.1.5:8000/api/boardings/${boardingId}`);
  ...
  const fetchTenantDetails = async () => {
    const tenantDetails = await Promise.all(
      tenants.map(async (tenantId) => {
        try {
          const response = await axios.get(`http://192.168.1.5:8000/api/user/user/${tenantId}`);
